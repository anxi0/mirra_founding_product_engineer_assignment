...
### 2-1. 수직/수평 접근이 동시에 가능한 UI 구조가 필요하다

IA 재설계에서 세로축(워크플로우)과 가로축(채널) 중 하나를 고르는 것이 일반적인 접근이지만, Mirra의 소상공인 타겟에서는 두 축을 모두 지원해야 합니다.

**수직 접근 — SNS별 독립 워크플로우:**
인스타그램 전용 기획 → 카드뉴스 생성 → 예약 → 성과를 하나의 세로 흐름으로 추적합니다. 채널마다 최적 포맷과 발행 주기가 다르기 때문에, SNS별로 기획-성과 사이클을 독립적으로 관리할 수 있어야 합니다.

**수평 접근 — 하나의 기획을 여러 SNS로:**
"이번 주 신제품 출시"라는 기획 하나를 인스타그램 카드뉴스 + TikTok 숏폼 + 카카오채널 텍스트로 동시에 변환하여 배포합니다. 소상공인은 동일한 사업 메시지를 여러 채널에 올려야 하는데, 지금은 채널마다 처음부터 다시 만들어야 합니다.

```
[수직 접근 — 채널 퍼스트]
인스타그램 선택 → 기획 → 카드뉴스 생성 → 예약 → 성과 확인
...
두 접근을 동시에 지원하는 UI는 기획 단계에서 "채널 하나"와 "여러 채널 동시"를 선택할 수 있도록 분기를 두면 됩니다. 현재 `/ko/planning`의 "계정을 선택하세요" 드롭다운이 단일 선택 구조인 것이 이 수평 접근을 막는 직접적인 구조적 부채입니다. 멀티 선택 + 채널별 포맷 자동 분기를 지원하도록 이 컴포넌트를 리팩토링하는 것이 가장 먼저 해야 할 IA 수정입니다.

**하네스 방향:** 단일 선택 드롭다운을 멀티 선택으로 바꾸기 전에, 현재 사용자 중 동일 주제로 여러 채널에 콘텐츠를 올리는 비율을 먼저 측정합니다. 이 수요가 확인되면 수평 접근 UI를 우선 구현하고, 수직 접근(채널 퍼스트 뷰)은 채널별 성과 데이터가 쌓인 후 추가합니다.
